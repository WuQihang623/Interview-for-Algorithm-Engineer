...
public:
    ...
    ListNode* sortList(ListNode* head) {
        priority_queue<ListNode*, vector<ListNode*>, my_compare> que;
        ListNode* cur = head;
        while(cur!=nullptr) {
            ListNode* tmp = cur->next;
            cur->next = nullptr;
...
public:
    ...
    ListNode* merge(ListNode* head1, ListNode* head2) {
        ListNode* dummpyHead = new ListNode(0);
        ListNode* tmp = dummpyHead, *tmp1 = head1, *tmp2 = head2;
...
public:
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
        ...
        while(i<m*n) {
            ans[i] = matrix[x][y];
            visited[x][y] = true;
            if (x+dx>=0&&x+dx<m&&y+dy>=0&&y+dy<n&&!visited[x+dx][y+dy]) {
                x += dx;
                y += dy;
            }
            else {
...
public:
    string decodeString(string s) {
        ...
        while(ptr < s.size()) {
            char cur = s[ptr];
            if (isdigit(cur)) { // 碰到了数字
                string digits = getDigits(s, ptr);
                stk.push_back(digits);
            }
            else if (isalpha(cur) || cur == '[') { //将字母或者'['进栈
...
public:
    // 寻找拓扑结构中是否有环
    // 对图中的节点有3中状态，未搜索，搜索中，已完成
    vector<int> visited; // 节点的状态，未搜索，搜索中，已完成
    vector<vector<int>> edges; // 节点之间的连接
    bool valid = true;
    void dfs(int u) {
        visited[u] = 1; // 当前节点正在搜索中
        for (int v: edges[u]) {
            if (visited[v]==0) {
                dfs(v);
                if (!valid) return;
...
```python
def match(pred, mask):
    match_matrix = np.zero((pred.max()-1, mask.max()-1))
    for i in range(1, pred.max()):
        for j in range(1, mask.max()):
            inter = np.logical_and(pred==i, mask==j).sum()
            union = np.logical_or(pred==i, mask==j).sum()
            iou = inter / union
            match_matrix[i][j] = iou
    paired = []
    while(match_matrix.max()>0):
        max_indices = np.where(matrix==np.max(match_matrix))[0]
        paired.append(max_indices)
        for i in range(pred.max()-1):
            match_matrix[i][max_indices[1]]=0
        for j in range(mask.max()-1):
			match_matrix[max_indices[0]][j]=0
    return paired
```
