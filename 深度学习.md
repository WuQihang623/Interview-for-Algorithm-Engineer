...
EdgeViTs：不在所有的Token上做Attention，挑选具有代表性的Token进行Attention的计算。

RiFormer：去除掉注意力机制，使用仿射变换替换MHSA，但是其计算、参数可以在训练完成后被吸收。但是需要大量的trick进行训练。

EfficientFormer：部分结构中可以去除掉MHSA。
